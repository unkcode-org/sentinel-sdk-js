...
12. The existing `window.error`/`unhandledrejection` listeners fan out one observation to OTel and RUM. RUM normalizes recognized `Error`/`DOMException` instances using a validated name (up to 128 bytes) and sanitized message (up to 1024 bytes). String rejection reasons may supply a sanitized message with `UnhandledRejection` as the type. Arbitrary objects, arrays, and other values are never inspected recursively or serialized; generic fallbacks remain. The existing URL text sanitizer removes query strings and fragments from absolute URLs. Messages still containing query/fragment markers or controls fall back. `stack` is permitted by Ingest but is deliberately omitted from this SDK change because it expands the privacy surface.
